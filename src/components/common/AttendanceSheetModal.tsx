...
export const AttendanceSheetModal: React.FC<AttendanceSheetModalProps> = ({ session, records, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm overflow-y-auto">
      <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden border border-slate-200 my-6">
        {/* Controls (Hidden during print) */}
        <div className="p-4 bg-slate-900 text-white flex items-center justify-between no-print">
          <div className="flex items-center gap-2">
            <FileSpreadsheet className="w-5 h-5 text-emerald-400" />
            <span className="font-bold text-sm">Official Municipal Attendance Sheet</span>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={handleExportCSV}
              className="px-3.5 py-1.5 bg-emerald-700 hover:bg-emerald-600 text-white text-xs font-semibold rounded-xl flex items-center gap-1.5 transition-colors shadow"
            >
        ...
        <div className="p-8 sm:p-12 bg-white text-slate-900 space-y-6">
          {/* Header */}
          <div className="text-center border-b-2 border-slate-900 pb-4">
            ...
            <h1 className="text-xl font-bold uppercase tracking-wider text-blue-950 font-display mt-1">
              {settings.orgName}
            </h1>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-slate-50 border border-slate-200 rounded-xl text-xs">
            <div>
              <span className="text-slate-500 font-medium block">Event / Activity:</span>
              <span className="font-bold text-slate-900 text-sm">{session.eventTitle}</span>
            </div>
            <div>
            ...
            <div>
              <span className="text-slate-500 font-medium block">Session Status:</span>
          ...
          <div className="grid grid-cols-5 gap-2 text-center text-xs">
            <div className="p-2 border border-slate-200 rounded-lg">
              <span className="text-slate-500 block text-[10px]">TOTAL REGISTERED</span>
              <span className="text-base font-bold text-slate-900">{session.totalRegistered}</span>
            </div>
            <div className="p-2 border border-emerald-200 bg-emerald-50 rounded-lg">
              <span className="text-emerald-700 block text-[10px]">PRESENT</span>
            ...
            <div className="p-2 border border-amber-200 bg-amber-50 rounded-lg">
              <span className="text-amber-700 block text-[10px]">LATE</span>
              <span className="text-base font-bold text-amber-800">{lateCount}</span>
            </div>
            <div className="p-2 border border-rose-200 bg-rose-50 rounded-lg">
              <span className="text-rose-700 block text-[10px]">ABSENT</span>
              <span className="text-base font-bold text-rose-800">{absentCount}</span>
            </div>
            <div className="p-2 border border-blue-200 bg-blue-50 rounded-lg">
              <span className="text-blue-700 block text-[10px]">ATTENDANCE RATE</span>
              <span className="text-base font-bold text-blue-900">{session.attendanceRate}%</span>
            </div>
          </div>
